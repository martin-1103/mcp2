 ...
 */
export async function handleExecuteFlow(args) {
    try {
        ...
        const endpoints = [];
        // Build flow steps
        const steps = flowData.flow_data.steps.map((step) => {
            if (step.endpointId) {
        ...
        if (flowData.flow_inputs) {
            try {
                ...
                const inputVars = {};
                flowData.flow_inputs.split(',').forEach((pair) => {
