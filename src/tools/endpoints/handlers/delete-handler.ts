...
import { EndpointDeleteResponse } from '../types.js';
 ...
 */
export async function handleDeleteEndpoint(args: Record<string, any>): Promise<McpToolResponse> {
  try {
    ...
    const result = await backendClient.makeRequest<EndpointDeleteResponse>(endpoint, {
      method: 'DELETE'
    });
    ...
    if (result.success && result.data) {
      ...
      const deleteText = formatEndpointDeleteText(
        endpointId,
        true,
        deleteData?.message || result.message
      );
